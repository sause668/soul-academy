        ...
        <div id="groupConC" className="whiteBox flex flex-col justify-flex-start items-start w-[80%] overflow-hidden">
            ...
            <div id="groupStudentsConC" className="flex flex-col justify-flex-start items-start w-full min-h-10">
                {group.students?.map((student, index) => (
                    <h3 id="groupStudentC" className={`text-md p-2 w-full ${index % 2 == 0 ? 'bg-blue-100' : 'bg-blue-50'} ${index < (group.students?.length ?? 0) - 1 ? 'border-b border-gray-300' : ''}`} key={`groupStudentC${index}`}>{student.lastName}, {student.firstName}</h3>
                ))}
            </div>
        </div>
    )
